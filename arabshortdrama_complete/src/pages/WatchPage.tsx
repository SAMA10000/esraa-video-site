...
useEffect(() => {
  ...
  return (
    <Layout>
      ...
      >
        ...
        >
          <div className="w-full h-full relative">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); handleChatToggle(e); }}
              className="absolute top-3 right-3 z-30 w-10 h-10 rounded-full bg-midnight/70 backdrop-blur-sm border border-gold/20 flex items-center justify-center text-gold touch-target"
            >
              {showChatOverlay ? <MessageCircleOff size={16} /> : <MessageCircle size={16} />}
            </button>
            ...
            {/* Live Chat Overlay */}
            {drama && <ChatOverlay drama={drama} visible={showChatOverlay} />}
      ...
      {/* Chat Input Sheet */}
      {drama && <ChatInputSheet isOpen={showChatInput} onClose={() => setShowChatInput(false)} drama={drama} />}
