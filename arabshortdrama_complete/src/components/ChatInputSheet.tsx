...
import { X, Send, Shield, Download, ChevronDown, Lock, CheckCircle } from 'lucide-react';
...
export function ChatInputSheet({ isOpen, onClose, drama }: ChatInputSheetProps) {
  ...
  const confirmName = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNameBackendStatus('تم تأكيد الاسم من الخادم. تقدري تكتبي والبوتات هترد عليك مباشرة.');
      void refreshMessages();
    } catch {
      ...
      >
        ...
        {!showGate ? (
          <div className="px-4">
            ...
            {confirmedName && (
              <div className="bg-gold/10 border border-gold/20 rounded-lg px-3 py-2 mb-3 text-right" dir="rtl">
                ...
                {!gateCompleted && <button type="button" onClick={() => setShowGate(true)} className="block mt-2 text-[11px] text-gold underline">فتح المشاهدة والدردشة المجانية بالكامل</button>}
              </div>
            )}
              ...
              <div className="max-h-56 overflow-y-auto space-y-2 mb-3 hide-scrollbar" dir="rtl">
                {messages.slice(-40).map((msg) => (
                  <div key={msg.id} className={`p-2 rounded-lg text-sm ${msg.isBot ? 'bg-midnight text-gray-300 text-right' : msg.isMine ? 'bg-gold/20 text-gold text-left' : 'bg-white/10 text-gray-200 text-right'}`}>
                    <span className="block text-[10px] text-gold/70 mb-1">{msg.username}{msg.isBot ? ' · بوت' : ''}</span>
                    {msg.text}
                  </div>
                ))}
                {waitingForReply && (
                  <div className="flex items-center gap-1 p-2" dir="rtl"><div className="flex gap-1"><div className="w-1.5 h-1.5 bg-gold rounded-full animate-bounce" /><div className="w-1.5 h-1.5 bg-gold rounded-full animate-bounce" style={{ animationDelay: '150ms' }} /><div className="w-1.5 h-1.5 bg-gold rounded-full animate-bounce" style={{ animationDelay: '300ms' }} /></div><span className="text-gray-500 text-xs">أحد البوتات يكتب...</span></div>
                )}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-2 pb-4">
              <input
                ...
                placeholder={confirmedName ? 'اكتب تعليقك والبوتات هترد عليك...' : 'أكدي اسمك أولاً قبل كتابة التعليق'}
          ...
          <div className="px-4 pb-8 pt-2">
            <button onClick={() => setShowGate(false)} className="absolute top-6 left-4 w-10 h-10 rounded-full bg-midnight flex items-center justify-center text-gray-400 touch-target"><ChevronDown size={24} /></button>
            <div className="text-center mb-5"><div className="w-16 h-16 rounded-full bg-gradient-gold flex items-center justify-center mx-auto mb-3 shadow-lg shadow-gold/30"><Lock size={32} className="text-midnight" /></div><h3 className="text-white font-bold text-lg mb-1" dir="rtl">فتح المشاهدة والدردشة المجانية بالكامل</h3></div>
            <div className="bg-midnight rounded-xl p-4 mb-4 text-right" dir="rtl"><p className="text-gray-300 text-sm leading-relaxed mb-3">مرحباً بك في منصتنا! لكي نستمر في تقديم الحلقات والمسلسلات القصيرة مجاناً وبأعلى جودة وبدون إعلانات مزعجة، يرجى إكمال الخطوة المجانية.</p></div>
            <div className="flex items-center justify-center gap-4 mb-4" dir="rtl"><div className="flex items-center gap-1 text-green-400 text-xs"><CheckCircle size={14} /><span>مجاني 100%</span></div><div className="flex items-center gap-1 text-green-400 text-xs"><Shield size={14} /><span>آمن</span></div><div className="flex items-center gap-1 text-green-400 text-xs"><Download size={14} /><span>خطوة بسيطة</span></div></div>
            <button onClick={handleCompleteOffer} className="w-full py-4 rounded-xl bg-gradient-gold text-midnight font-bold text-base touch-target animate-pulse-gold">إكمال المهمة وفتح المنصة مجاناً</button>
            <p className="text-gray-500 text-[10px] text-center mt-3" dir="rtl">آمن 100% • بدون سحب رصيد • مجاني بالكامل</p>
          </div>
        )}
      </div>
    </>
