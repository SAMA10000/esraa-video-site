...
$violation = message_violation($message);
    ...
    // Spam detection
    $spamReason = detect_spam_behavior($visitor, $message);
    if ($spamReason) {
        auto_ban_visitor($visitor, $spamReason);
        json_response(['success'=>false,'error'=>'تم اكتشاف سلوك غير طبيعي. تم حظرك مؤقتاً.'], 429);
    }
...
try {
    ...
    // Trigger bot reply immediately after visitor message
    try {
      $replyBody = ['drama_id' => $drId, 'user_text' => $message, 'reply_to_message_id' => $id];
      $botReply = bot_create_contextual_message($replyBody, 'bot_direct_reply', $id, 'reply');
      if ($botReply) {
        $botStmt = db()->prepare("INSERT INTO chat_messages (username, message, session_id, visitor_id, drama_id, category_id, reply_to_message_id, bot_profile_id, status, is_bot, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'visible', 1, NOW())");
        $botStmt->execute([
          $botReply['username'],
          $botReply['text'],
          'bot_' . bin2hex(random_bytes(8)),
          $visitor['visitor_id'] ?? null,
          $drId,
          $catId,
          $id,
          $botReply['profile']['id'] ?? null
        ]);
        $botReply['dbId'] = (int)db()->lastInsertId();
      }
    } catch (Throwable $e) {}
